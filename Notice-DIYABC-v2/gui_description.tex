...
\includegraphics[scale=0.35]{gui_pictures/Capture-DIYABC-52.png} \\
...
\includegraphics[scale=0.35]{gui_pictures/Capture-DIYABC-53.png} \\
...
Note that when the set of summary statistics is changed (as here), it is necessary to also simulate a large number of datasets using priors to get a new reference table.\\
 We validate twice and launch the analysis. When it is finished, we click on the  \fbox{\textsf{View results}} button and get this screen:\\
...
The first tab (on the left) is related to the use of a computer cluster to perform computations of the reference table. If your computer is connected with a computer cluster on which the necessary files (detailed in section 5) have been correctly installed, and if the computer cluster is run under SGE queuing system, then you can use it for generating the reference table. You then need to :
\begin{enumerate}
 \item check the box \textsf{Use a cluster (...)}
 \item indicate the number of data sets produced by each single job of the queue
 \item give the name of the SGE queue 
\end{enumerate}

Clicking on the \fbox{\textsf{Run computation}} button will send the right script to the computer cluster, monitor the progression of computations and retrieve the reference table once all computations are done.
