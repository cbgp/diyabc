...
The header file, always named \texttt{header.txt}, contains all information
necessary to compute a reference table associated with the data :
i.e. the scenarios, the scenario parameter priors, the characteristics
of loci, the loci parameter priors and the summary statistics to compute.
As soon as the first records of the reference table have been saved
in the reference table file, always named \texttt{reftable.bin} and
also included in the project directory, the project is "locked".
This means that the header file can not be changed anymore. If one
needs to change a scenario or a parameter prior, or a summary statistics,
a new project needs to be defined. This is to guarantee that all subsequent
actions performed on the project are in coherence with the current
data and header files. It is of course strongly advised NOT to move
files among projects. Incidentally, the \texttt{header.txt} file is
only built when the project has been saved, the information progressively
input by the user being saved in a series of temporary files.\\


Once a sufficiently large reference table has been simulated, analyses
can be performed. Their different output files are copied to the \emph{analysis}
directory included in the project directory, and containing as many
directories as analyses performed. Hence, it is now much easier to
know with certainty the conditions of each analysis.


\subsection{Options of the home screen}

\label{doc_newSNPProjectButton} The home screen above has two menus
and several buttons.\\
...
\includegraphics[scale=0.4]{gui_pictures/Capture-DIYABC-4}\\
...
Let's enter \textbf{demo1} as the project name and click on the \fbox{\textsf{Create
project}} button.\\
 The following screen appears :\\


\includegraphics[scale=0.35]{gui_pictures/Capture-DIYABC-8}
...
by clicking on the corresponding \fbox{\textsf{Browse}} button (previous
screen). The usual file browsing screen appears (below) and one has
to select a Genepop format data file, here \texttt{data1.mss}. \\


\includegraphics[scale=0.35]{gui_pictures/Capture-DIYABC-9} \\
 Clicking on the \fbox{\textsf{Open}} button leads to the following
...
Click on the corresponding \fbox{\textsf{Set}} button. The following
...
on the \fbox{\textsf{Define priors}} button. We get this :\\


\includegraphics[scale=0.35]{gui_pictures/Capture-DIYABC-12}

The parameter prior frame allows to choose the prior density of each
...
If we click on the \fbox{\textsf{Check scenario}} button, the logic
...
The scenario can be saved by clicking on the \fbox{\textsf{SAVE}}
button. The frame can be close by clicking on the \fbox{\textsf{CLOSE}}
button.\\


Since the scenario has been checked, we can validate and save the
historical model by clicking on the \fbox{\textsf{VALIDATE AND SAVE}}
...
Click on the corresponding \fbox{\textsf{Set}} button. We get the
following screen : \\
...
the \fbox{\textsf{Add group}} button. We get this :\\


\includegraphics[scale=0.35]{gui_pictures/Capture-DIYABC-16}
...
\includegraphics[scale=0.35]{gui_pictures/Capture-DIYABC-18}

Note that the \fbox{\textsf{Auto group}} button would have produced
...
of the locus group. Clicking on the \fbox{\textsf{Set mutation model}}
button, the following screen appears :\\


\includegraphics[scale=0.35]{gui_pictures/Capture-DIYABC-19}

Once the mutation model of Group 1 is defined, we click on the \fbox{\textsf{VALIDATE}}
button to go back to the previous screen. Clicking on the \fbox{\textsf{Set
Summary statistics}} button, we get the following screen :\\
...
Once finished, we click on the \fbox{\textsf{VALIDATE}} button to
...
Then click on the \fbox{\textsf{Run computations}} button. If things
...
time (at the left of the \fbox{\textsf{Run computations}} button):\\
...
on the \fbox{\textsf{Define new analysis}} button and get this new
screen :\\
...
After clicking on the \fbox{\textsf{VALIDATE}} button, we go back
...
a way to copy an analysis (\fbox{\textsf{Copy}} button), to make
some modifications (\fbox{\textsf{Edit}} button) before launching
it or to delete the analysis (\fbox{\textsf{Del}} button).\\


\includegraphics[scale=0.35]{gui_pictures/Capture-DIYABC-30} \\


Let's click on the \fbox{\textsf{Launch}} button. This analysis
...
To view results, just click on the \fbox{\textsf{View results}}
button. After some seconds (while the program reads the PCA result
file), we can see this : \\


\includegraphics[scale=0.35]{gui_pictures/Capture-DIYABC-32} \\
...
further the results of the PCA.\\
 The graphic can be printed or saved (\fbox{\textsf{PRINT}} and
\fbox{\textsf{SAVE}} buttons, respectively). Clicking on the \fbox{\textsf{CLOSE}}
button closes the result window. Eventually, clicking on the \fbox{\textsf{View
numerical results}} opens up another screen as shown below :\\
...
As usual, results can be printed (\fbox{\textsf{PRINT}}) and/or
saved (\fbox{\textsf{SAVE}}). Click on \fbox{\textsf{OK}} to leave
this screen.\\
...
Back on the screen of page 30, we click on the \fbox{\textsf{Define
new analysis}} button. We choose the \texttt{Estimate posterior distribution
...
We click on the \fbox{\textsf{VALIDATE}} button and get the following
...
do than to click on the \fbox{\textsf{VALIDATE}} button : \\


\includegraphics[scale=0.35]{gui_pictures/Capture-DIYABC-35} \\
...
and click on the \fbox{\textsf{VALIDATE}} button. We get back to
the Analysis control panel which now looks like this:\\


\includegraphics[scale=0.35]{gui_pictures/Capture-DIYABC-38} \\


%\newpage
We click on the \fbox{\textsf{Launch}} button. The analysis progress
is now visible :\\
...
on the \fbox{\textsf{Stop}} button. Once this second analysis is
finished, we can view its results by clicking on the \fbox{\textsf{View
results}} button :\\


\includegraphics[scale=0.35]{gui_pictures/Capture-DIYABC-39} \\
...
appears when clicking on the upper left \fbox{\textsf{view numerical
results}} button, showing this :\\


\includegraphics[scale=0.35]{gui_pictures/Capture-DIYABC-41} \\


We go back to the previous screen by cliking the \fbox{\textsf{OK}}
...
Let's define a new analysis (click on the \fbox{\textsf{Define new
analysis}} button) and choose the option \texttt{Compute bias and
...
\includegraphics[scale=0.4]{\string"gui_pictures/Capture du 2015-06-27 15:25:00 bias1 screen1\string".png}
\\
...
(ii) they are drawn drom the initial prior distributions (which can
...
default)'' and on the \fbox{\textsf{VALIDATE}} button:\\
...
on \fbox{\textsf{VALIDATE}} which brings us to the following screen
:\\


\includegraphics[scale=0.35]{gui_pictures/Capture-DIYABC-47} \\


If we want to keep the same distributions for mutation parameters
as when builduing the reference table, we just click on \fbox{\textsf{VALIDATE}}.
If we need to change them, we click on \fbox{\textsf{Set mutation
model}} which would bring the following screen :\\
...
Accuracy measures are available for original, composite and scaled
...
(again) on the \fbox{\textsf{Define new analysis}} button) and choose
...
drawn from posterior distributions'' and on the \fbox{\textsf{VALIDATE}}
button.

\includegraphics[scale=0.4]{\string"gui_pictures/Capture du 2015-06-27 15:47:39 bias2 screen1\string".png}
...
\includegraphics[scale=0.4]{\string"gui_pictures/Capture du 2015-06-27 15:51:23 bias2 screen 2\string".png}
...
\includegraphics[scale=0.4]{\string"gui_pictures/Capture du 2015-06-27 16:00:31 bias 2 screen 3\string".png}
...
click on the \fbox{\textsf{View results}} button.

\includegraphics[scale=0.4]{\string"gui_pictures/Capture du 2015-06-27 17:22:43 bias2 screen 5\string".png}
...
\includegraphics[scale=0.32]{gui_pictures/Capture-DIYABC-53} \\
...
is also a button \fbox{\textsf{Redefine summary statistics of group:}}
...
it is finished, we click on the \fbox{\textsf{View results}} button
and get this screen:\\
...
Clicking on the \fbox{\textsf{View numerical results}} leads to
...
\includegraphics[scale=0.25]{\string"gui_pictures/Capture du 2015-06-29 16:51:52 scenarios 1 et 2\string".png}
...
\includegraphics[scale=0.25]{\string"gui_pictures/Capture du 2015-06-29 16:36:01 prior demo3\string".png}
...
seconds. After clicking on the \fbox{\textsf{VALIDATE}} button,
...
\includegraphics[scale=0.3]{gui_pictures/Capture-DIYABC-109} \\
...
a few seconds and press on the \fbox{\textsf{View results}} button.
The following screen appears:\\


\includegraphics[scale=0.3]{gui_pictures/Capture-DIYABC-111} \\
...
results}}, the program shows a subset of numerical values used in
...
\includegraphics[scale=0.4]{\string"gui_pictures/Capture du 2015-06-29 09:37:34 conf1 screen1\string".png}\\
...
\includegraphics[scale=0.4]{\string"gui_pictures/Capture du 2015-06-29 09:48:18 conf1 screen2\string".png}
...
\includegraphics[scale=0.4]{\string"gui_pictures/Capture du 2015-06-29 09:55:46 conf1 screen3\string".png}\\
...
\includegraphics[scale=0.4]{\string"gui_pictures/Capture du 2015-06-29 10:20:32 conf1 screen4\string".png}
...
\includegraphics[scale=0.4]{\string"gui_pictures/Capture du 2015-06-29 10:47:13 conf2 screen1\string".png}
...
\includegraphics[scale=0.4]{\string"gui_pictures/Capture du 2015-06-29 10:50:57 conf2 screen3\string".png}

When clicking and validating the GLOBAL (prior error rate) option
we go to the following screen.

\includegraphics[scale=0.4]{\string"gui_pictures/Capture du 2015-06-29 10:58:48 conf2 screen4\string".png}
...
\includegraphics[scale=0.4]{\string"gui_pictures/Capture du 2015-06-29 11:23:35 conf2 screen5\string".png}
...
\includegraphics[scale=0.4]{\string"gui_pictures/Capture du 2015-06-29 12:20:38 conf3 screen2\string".png}

After validating default values for historical and mutational parameters,
we launch the analysis. When it is done, we click on the \fbox{\textsf{View
results}} button and get the following screen:\\


\includegraphics[scale=0.4]{\string"gui_pictures/Capture du 2015-06-29 12:24:48 conf3 screen4\string".png}\\
...
\includegraphics[scale=0.4]{\string"gui_pictures/Capture du 2015-06-29 12:26:01 conf3 screen6\string".png}\\
...
We first inform the historical model clicking on the \fbox{\textsf{Set}}
...
We click on the \fbox{\textsf{Set parameter values}} button. Arbitrary
...
We change these values according to our needs and we click the \fbox{\textsf{Set
sample size}} button, getting this screen:\\
...
Clicking on the \fbox{\textsf{VALIDATE}} button, we get back to
...
We have now to complete the Genetic data (click on the \fbox{\textsf{Set}}
...
We click on the \fbox{\textsf{OK}} button and get the following
screen :\\
...
Since mutation models are different for microsatellites and DNA sequences,
we define two groups by clicking twice on the \fbox{\textsf{Add group}}
button :\\
...
The six locus names are transferred into group 1 by clicking on the
\fbox{\textsf{$>>$}} button :\\
...
2.4). Let's click on the \fbox{\textsf{Set Mutation Model}} button
of group 1:\\
...
\fbox{\textsf{VALIDATE}} button. Then we set the mutation model
...
\newpage{}We then click on the \fbox{\textsf{Run computation}}
...
program. When the user clicks on buttons such as \fbox{\textsf{Run
computation}} or \fbox{\textsf{Launch}}, the graphic interface
...
version of the executable) by clicking on the \fbox{\textsf{browse}}
button.\\
...
\includegraphics[scale=0.33]{gui_pictures/Capture-DIYABC-101} \\



\subsubsection{Tab \textsf{``cluster''}}

\label{clustergui} The third tab is related to the use of a computer
...
\end{enumerate}
Clicking on the \fbox{\textsf{Run computation}} button generates
