...
gene sampling, respectively. %Eventually, only for SNPs, we have added the keyword \texttt{refsample} to control the ascertainment bias (see below).
\\
...
\end{description}
A historical model is a succession of lines as described above. However,
...
Although a variety of mutation models have been proposed for microsatellite
...
It is worth stressing that, using the Hudson\textquoteright{}s simulation
...
The Bayesian aspect of the ABC approach implies that parameter estimations
...
Data simulation is based on the Wright-Fisher model. It consists in
...
Several steps are necessary to get posterior distributions of parameters.
...
As stressed in previous studies \citep[e.g.][]{Ex2005}, the ABC appproach
...
\item [{the 50% and 95% coverages}] : the proportion of test data sets
...
The program DIYABC provides two estimates of the posterior probability
of each scenario :
\begin{description}
