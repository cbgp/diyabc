...
int main(int argc, char *argv[]){
	...
	if((test_file != NULL) && (force == false)){
		cout << "Some files saving the RNG states already exist. Use -f if you want to overwrite it." << endl;
		return 0;
	}
	test_file.close();
	...
	mtss = get_mt_parameters_st(32, 521, 0, (num_nodes*num_threads)-1, 4172, &countRNG);
	// Fixons les graines
